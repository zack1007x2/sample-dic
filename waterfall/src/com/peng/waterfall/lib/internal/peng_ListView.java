...
package com.peng.waterfall.lib.internal;

import java.util.ArrayList;

import com.peng.waterfall.activity.R;
 ...
 */
public class peng_ListView extends peng_AbsListView {
    ...
    private ArrayList<FixedViewInfo> mHeaderViewInfos = new ArrayList<peng_ListView.FixedViewInfo>();
    private ArrayList<FixedViewInfo> mFooterViewInfos = new ArrayList<peng_ListView.FixedViewInfo>();
    ...
    public peng_ListView(Context context) {
        this(context, null);
    }

    public peng_ListView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.listViewStyle);
    }

    public peng_ListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public boolean removeHeaderView(View v) {
        if (mHeaderViewInfos.size() > 0) {
            ...
            if (((peng_HeaderViewListAdapter) mAdapter).removeHeader(v)) {
    ...
    public boolean removeFooterView(View v) {
        if (mFooterViewInfos.size() > 0) {
            boolean result = false;
            if (((peng_HeaderViewListAdapter) mAdapter).removeFooter(v)) {
                mDataSetObserver.onChanged();
                result = true;
            }
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (mHeaderViewInfos.size() > 0 || mFooterViewInfos.size() > 0) {
            mAdapter = new peng_HeaderViewListAdapter(mHeaderViewInfos, mFooterViewInfos, adapter);
        } else {
            mAdapter = adapter;
        }
    ...
    final int measureHeightOfChildren(int widthMeasureSpec, int startPosition, int endPosition, final int maxHeight,
        ...
        final peng_AbsListView.RecycleBin recycleBin = mRecycler;
    ...
    private void setupChild(View child, int position, int y, boolean flowDown, int childrenLeft, boolean selected, boolean recycled) {
        ...
        peng_AbsListView.LayoutParams p = (peng_AbsListView.LayoutParams) child.getLayoutParams();
        if (p == null) {
            p = new peng_AbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT, 0);
        }
        p.viewType = mAdapter.getItemViewType(position);

        if ((recycled && !p.forceAdd) || (p.recycledHeaderFooter && p.viewType == peng_AdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER)) {
            attachViewToParent(child, flowDown ? -1 : 0, p);
        } else {
            p.forceAdd = false;
            if (p.viewType == peng_AdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER) {
    ...
    private void scrollListItemsBy(int amount) {
        ...
        if (amount < 0) {
            ...
            while (first.getBottom() < listTop) {
                peng_AbsListView.LayoutParams layoutParams = (LayoutParams) first.getLayoutParams();
            ...
            while (last.getTop() > listBottom) {
                peng_AbsListView.LayoutParams layoutParams = (LayoutParams) last.getLayoutParams();
                if (recycleBin.shouldRecycleViewType(layoutParams.viewType)) {
